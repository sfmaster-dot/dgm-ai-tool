...
export const MENUOPTION_GUIDES = [
  ...
  { 
    n: '4', 
  title: "옵션설명 = 혜택 명확화", 
  desc: '단순 메뉴 안내 ❌, "왜 이 옵션을 추가해야 하는가 = 구매 동기"가 핵심. "추가 옵션" ❌ → "메인 주문 시에만 드리는 가격!" ✅. "사이드 메뉴 추가" ❌ → "단품 4,000원 → 지금 2,900원!" ✅. 가격·가성비·조건부 한정 중 하나는 반드시 넣습니다. ⚠️ 할인율(%)은 실제로 깎아준 자리에만. 단품가 없이 "30% 저렴한!"이라고 쓰면 고객이 계산해 봅니다.', 
  },
  {
    n: '5',
  ...
  },
  {
    ...
    desc: "안정기에는 '판매가 4,000원짜리를 무료 제공' → '2,900원에 제공'으로 전환합니다. 원 판매가에서 25~27% 할인이 안전선. 더 깎으면 '원래 그 값이었네'가 되어 정가 판매가 안 됩니다. 사장님은 마진을 회복하고, 고객은 '싸게 산 진짜 메뉴'로 인식해 만족도와 재주문률이 같이 오릅니다.",
  },
  {
    n: '7',
    title: '⭐ 배민 옵션 할인 — 취소선을 붙여야 할인이 보입니다',
    desc: "할인가를 옵션 칸에 그냥 낮춰 넣으면 그냥 싼 옵션일 뿐입니다. [배민셀프서비스 → 메뉴별 할인 관리 → 옵션할인]에서 걸어야 정가에 취소선이 그어지고 할인율이 붙습니다. 등록가 4,000원 + 할인 1,100원 = 2,900원 이런 식입니다. • 할인액은 100원 단위, 등록가의 5% 이상이면서 최소 300원 • 할인 후 최종 200원 이상(무료 불가 — 무료는 +0원 옵션으로 따로) • 500원 미만 옵션은 대상 아님 • 한 번에 40개까지 • 요일·시간 설정은 없음 • 가격을 수정했다면 48시간 뒤에 등록됩니다. 가격 손보실 계획이면 이틀 앞서 하십시오.",
  },
];
