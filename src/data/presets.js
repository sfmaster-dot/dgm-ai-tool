...
export const MENUOPTION_GUIDES = [
  ...
  { 
    n: '4', 
  title: "옵션설명 = 혜택 명확화", 
  desc: '단순 메뉴 안내 ❌, "왜 이 옵션을 추가해야 하는가 = 구매 동기"가 핵심. "추가 옵션" ❌ → "메인 주문 시에만 드리는 가격!" ✅. "사이드 메뉴 추가" ❌ → "단품 4,000원 → 지금 1,900원!" ✅. 가격·가성비·조건부 한정 중 하나는 반드시 넣습니다. ⚠️ 할인율(%)은 실제로 깎아준 자리에만. 단품가 없이 "30% 저렴한!"이라고 쓰면 고객이 계산해 봅니다.', 
  },
  {
    n: '5',
  ...
  },
  {
    ...
    desc: "리뷰이벤트는 대부분 무료로 줍니다. 무료로 주면 원가만큼 그대로 손해입니다. 원가 1,000원짜리를 월 500건 뿌리면 50만원이 날아갑니다. 기준은 하나입니다 — 무료로 주지 말고, 원가 건지고 500원은 남기십시오. 전환가 = (원가 + 500원) ÷ 0.88 (0.88은 수수료 10.8%×1.1을 뺀 몫). 예를 들어 6,000원짜리 원가가 1,000원이면 1,700원을 받으시면 됩니다. 무료일 때 −1,000원이 +498원으로 바뀝니다. 건당 1,500원 차이입니다. 할인폭이 커 보여도 괜찮습니다. 무료는 사은품이지만 취소선 붙은 1,700원은 횡재로 읽힙니다.",
  },
  {
    n: '7',
    title: '⭐ 배민 옵션 할인 — 취소선을 붙여야 할인이 보입니다',
    desc: "할인가를 옵션 칸에 그냥 낮춰 넣으면 그냥 싼 옵션일 뿐입니다. [배민셀프서비스 → 메뉴별 할인 관리 → 옵션할인]에서 걸어야 정가에 취소선이 그어지고 할인율이 붙습니다. 등록가 6,000원 + 할인 4,300원 = 1,700원 이런 식입니다. • 할인액은 100원 단위, 등록가의 5% 이상이면서 최소 300원 • 할인 후 최종 200원 이상(무료 불가 — 무료는 +0원 옵션으로 따로) • 500원 미만 옵션은 대상 아님 • 한 번에 40개까지 • 요일·시간 설정은 없음 • 가격을 수정했다면 48시간 뒤에 등록됩니다. 가격 손보실 계획이면 이틀 앞서 하십시오.",
  },
];
